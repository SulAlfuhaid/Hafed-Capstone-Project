    ...
    <div class="container">
        <div class="d-flex justify-content-between mb-3 mt-4">
            ...
            <div>
                ...
                <form action="{{ route('student.notifications.deleteAll') }}" method="POST" class="d-inline">
                    @csrf
                    ...
                    <div class="col-md-12">
                        <div class="card shadow-sm mb-3 {{ !$notification->is_read ? 'bg-light' : 'bg-white' }}">
                            <div class="card-body d-flex align-items-center">
                                <div class="me-3">
                                    <i class="fas {{ $notification->type == 'تقرير جديد' ? 'fa-file-alt' : 'fa-bell' }} fa-2x text-primary"></i>
                                </div>
                                <div class="flex-grow-1">
                                    <h5 class="card-title mb-1">{{ $notification->type }}</h5>
                                    @if($notification->type == "تقرير جديد")
                                        <a href="{{ route('student.reports') }}" class="text-primary">عرض التقرير</a>
                                    @elseif($notification->type == "تقييم جديد")
                                        <a href="{{ route('student.evaluations') }}" class="text-primary">عرض التقييم</a>
                                    @else
                                ...
                                <div class="text-end">
                                    ...
                                    <form action="{{ route('student.notifications.read', $notification->notification_id) }}" method="POST" class="d-inline">
                                        @csrf
                                    ...
                                    <form action="{{ route('student.notifications.delete', $notification->notification_id) }}" method="POST" class="d-inline">
                                        @csrf
